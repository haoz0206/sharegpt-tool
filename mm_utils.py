...
def _read_video_torchvision(
    ...
    if version.parse(torchvision.__version__) < version.parse("0.19.0"):
        if "http://" in video_path or "https://" in video_path:
            warnings.warn("torchvision < 0.19.0 does not support http/https video path, please upgrade to 0.19.0.", stacklevel=2)
...
(at {video_start_clamped if video_start is not None else 0}s) "
            f"exceeds end frame {end_frame} (at {video_end_clamped if video_end is not None else max_duration}s). "
            f"Video duration: {max_duration:.2f}s ({total_frames} frames @ {video_fps}fps)"
        )

    logger.info(
        f"calculate video frame range: {start_frame=}, {end_frame=}, \
{total_frames=} from {video_start=}, {video_end=}, {video_fps=:.3f}"
    )
...
def fetch_video(
    ...
    if isinstance(ele["video"], str):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(fetch_image, {"image": video_element, **process_info}, image_factor) for video_element in ele["video"]]
        ...
        for message in conversation:
            if isinstance(message["content"], list):
                for ele in message["content"]:
                    if "image" in ele or "image_url" in ele or "video" in ele or ele.get("type", "text") in ("image", "image_url", "video"):
...
def process_vision_info(
    ...
    return_video_metadata: bool = False,
    image_patch_size: int = 14,
) -> tuple[Optional[list[Image.Image]], Optional[list[torch.Tensor | list[Image.Image]]], Optional[dict[str, Any]]]:
